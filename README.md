# Christopher Vistal

**Full-Stack Software Engineer** — I build production systems for government agencies, law firms, and enterprise teams.

Case management platforms, citizen records systems, and AI-powered tools that ship and stay shipped.

📍 Surigao del Sur, Philippines &nbsp;·&nbsp; 🌐 [chris-dev.netlify.app](https://chris-dev.netlify.app) &nbsp;·&nbsp; 📧 vistalchris@gmail.com

---

## 🏗️ Featured Work

| Project | What it does | Stack |
|---------|-------------|-------|
| [ATA Law Firm](https://atafirm.ph) | Case management system — case tracking, document automation, client portal | Laravel, Vue.js, AWS |
| [NEMSU Data Hub](https://udh.nemsu.edu.ph) | Live university data platform — student & faculty records | Laravel, Vue.js, MySQL |
| [Dragonica Spidpex](https://dragonicaspidpex.com) | Live gaming platform with player accounts, leaderboards & community | Laravel, Vue.js |
| Offline AI Transcriber | Local speech-to-text, real-time waveform feedback — zero cloud dependency | Python, CTranslate2, Whisper |
| GSIS Deduction System | Government insurance deduction engine for civil servants | PHP, SQL Server |

---

## 🧰 Stack

**Backend** &nbsp;·&nbsp; PHP / Laravel · Node.js · MySQL · SQL Server · REST APIs · Socket.io

**Frontend** &nbsp;·&nbsp; Vue.js · TypeScript · Tailwind CSS · Inertia.js

**AI & ML** &nbsp;·&nbsp; Python · CTranslate2 · Whisper · NLP · Speech-to-Text

**DevOps** &nbsp;·&nbsp; AWS (EC2, S3, RDS) · Docker · Linux · CI/CD

**Other** &nbsp;·&nbsp; Java (Android) · Go · C# (.NET)

---

## 💼 Experience

- **Freelance Software Engineer** — 2023–present
- **Software Engineer**, Province of Surigao del Sur — 2021–2023
- **Intern Developer**, N.E. Mindanao State University — 2018

5+ years building production systems · 3,000+ end users · 9 live products

---

## 📫 Hire Me

Open to remote contracts, full-time, and part-time engagements.

**[chris-dev.netlify.app](https://chris-dev.netlify.app)** &nbsp;·&nbsp; vistalchris@gmail.com
